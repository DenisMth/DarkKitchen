...
function create_cards(list, type) {
  list.forEach((food, index) => {
    ...
    foodPrice.addEventListener("click", function () {
      console.log("la function fonctionne");
      let section = this.parentNode;
      let food_Name_achat = section.querySelector("h2");
      achat.push(food_Name_achat.textContent);
      solde.push(parseFloat(food.price));
      console.log(achat);
      console.log(solde);
    });
...
function create_button_display() {
  let button = document.createElement("button");
  button.className = "dropdown";
  let list_achat = document.createElement("div");

  button.addEventListener("click", function () {
    list_achat.style.display = (list_achat.style.display === 'block') ? 'none' : 'block';
    list_achat.innerHTML = `<img src="assets/resources/images/cart.svg">`;

    for (let elem of achat) {
      let my_achat = document.createElement("div");
      my_achat.textContent = elem;
      list_achat.appendChild(my_achat);
    }
    let sum_solde = document.createElement("div");
    let sum = 0;
    for (let i = 0; i < solde.length; i++) {
      sum += solde[i];
    }
    sum_solde.textContent = "Total: " + sum;
    list_achat.appendChild(sum_solde);
  });
  button.appendChild(list_achat);
  document.querySelector("header").appendChild(button);
}
...
function create_Order() {
  ...
  order.id = "order_Form";
  document.querySelector("header").appendChild(order);
