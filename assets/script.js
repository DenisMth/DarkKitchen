...
// Créer un conteneur pour le panier et l'ajouter au corps du document
...
// Créer une liste non ordonnée pour les articles dans le panier et l'ajouter au conteneur
const panierListe = document.createElement('ul');
panierListe.id = 'panier';
panierContainer.appendChild(panierListe);

// Créer un élément de paragraphe pour afficher le coût total et l'ajouter au conteneur
...
// Fonction pour afficher les articles dans le panier et mettre à jour le coût total
function afficherPanier() {
    ...
    panierItems.forEach((food) => {
        const listItem = document.createElement('li');
        listItem.innerText = `${food.name} - Prix : €${food.price.toFixed(2)}`;
...
document.addEventListener('DOMContentLoaded', function () {
    // Ajouter un gestionnaire d'événements à chaque bouton 'foodPrice'
    document.querySelectorAll('.foodPrice').forEach((bouton) => {
        bouton.addEventListener('click', () => {
            // Récupérer le nom et le prix du plat à partir des attributs de données
            const foodName = bouton.dataset.nom;
            const foodPrice = parseFloat(bouton.dataset.prix);

            // Créer un objet représentant le plat et l'ajouter au panier
            ajouterAuPanier({ name: foodName, price: foodPrice });
        });
    });
});
console.log(total)